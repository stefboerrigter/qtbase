/*
 * Minor tweak to bytebit[] for VNC compatibility, fix prototypes
 * Public function names changed to avoid name clash
 */
...
void vnc_deskey(unsigned char *key, short edf)	/* Thanks to James Gillogly & Phil Karn! */
{
	int i, j, l, m, n;
...
void vnc_cpkey(unsigned long *into)
{
...
void vnc_usekey(unsigned long *from)
{
...
void vnc_des(unsigned char *inblock, unsigned char *outblock)
{
...
short mode;
{
	...
	vnc_deskey(&hexkey[8], revmod);
	vnc_cpkey(KnR);
	vnc_deskey(hexkey, mode);
	vnc_cpkey(Kn3);					/* Kn3 = KnL */
	return;
	}
...
short mode;
{
	...
	vnc_deskey(&hexkey[8], revmod);
	vnc_cpkey(KnR);
	vnc_deskey(third, mode);
	vnc_cpkey(Kn3);
	vnc_deskey(first, mode);
	return;
	}
